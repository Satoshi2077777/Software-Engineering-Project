...
import com.view.ThingDialogView;

// 可以有多个类。  
class sm extends JFrame {
	TextField t;
...
public class Frame extends JFrame implements ActionListener {
	...
	*/
	...
	public String getText() {
		if (f != null) {
			f.jtf_thtext.setText(textArea1.getText());
			return textArea1.getText();
		} else
			return null;
	}

	ThingDialogView f = null;

	public Frame(ThingDialogView f, String text) {
		this.f = f;
		...
		leftone.setAccelerator(
				KeyStroke.getKeyStroke(KeyEvent.VK_L, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		JMenuItem rightone = new JMenuItem("右半屏");
		rightone.setAccelerator(
				KeyStroke.getKeyStroke(KeyEvent.VK_R, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		JMenuItem max = new JMenuItem("最大化");
		max.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Z, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		JMenuItem min = new JMenuItem("最小化");
		min.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_I, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
	...
	private class help extends JDialog {
		public help() {
			...
			JLabel label1 = new JLabel("sm通知编辑", JLabel.CENTER); // JLabel.CENTER可以实现将label文字放在居中位置
			JLabel label2 = new JLabel("如有任何问题请您咨询sm", JLabel.CENTER);
			Font font = new Font(Font.DIALOG, Font.PLAIN, 20); // 设置字体！
